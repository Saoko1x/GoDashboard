...
export default function Component({ trainingId }: { trainingId: any }) {
  const utils = trpc.useContext();

  const trainingIdNumber = Number(trainingId);

  const training = trpc.training.getById.useQuery({
    id: trainingIdNumber
  });

  const deleteTask = trpc.trainingTask.delete.useMutation({
    onSuccess: () => {
      utils.training.getById.invalidate({ id: trainingIdNumber });
    }
  });
  ...
  const { isLoading } = trpc.training.getById.useQuery({
    id: trainingIdNumber
  });
